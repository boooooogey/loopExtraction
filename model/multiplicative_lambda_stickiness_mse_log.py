...
def model(start_diag:int, end_diag:int):
    ...
    def contactmap(u_0:ArrayLike,
        ...
        @jax.jit
        def _inner(carry, x):
            left, right, slowdown_left, slowdown_right, s_left, s_right, prev_state = carry # ith diagonal
            ...
            s_left = jnp.roll(s_left, 1)
            curr_state = nom - denom
            return (left, right, slowdown_left, slowdown_right, s_left, s_right, curr_state), logsumexp(curr_state, s_left * s_right)
        ...
        mat = jax.lax.scan(_inner,
                           ...
                           None,
                           length=n)[1]
        return jnp.vstack([logsumexp(u_0, stickiness * stickiness), mat])
    ...
    def model_init(mat:ArrayLike) -> Parameters:
        ...
        stickiness = jnp.ones(width) * 0.01
        return u_0, p_r, p_l, lmbd, stickiness
    ...
    def total_loss(mat:ArrayLike,
                   ...
                   p_l:ArrayLike,
                   lmbd:ArrayLike,
                   stickiness:ArrayLike) -> float:
        mat_pred = contactmap(u_0, p_r, p_l, lmbd, stickiness)
